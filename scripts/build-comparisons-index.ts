...
function buildComparisonsIndex(): void {
  ...
  for (const runId of entries) {
    ...
    // Prefer a frame from the actual take over a shared shot grid or an MP4
    // accidentally used as an image. Posters are generated by build:posters.
    for (const artifact of artifacts) {
      if (!['video_result', 'end_video'].includes(artifact.artifact_type) || !artifact.media_url) continue;
      const prefix = `/data/comparisons/${runId}/media/`;
      if (!artifact.media_url.startsWith(prefix)) continue;
      const filename = artifact.media_url.slice(prefix.length);
      if (filename.includes('/') || filename.includes('..')) continue;
      if (existsSync(join(runDir, 'media', `${filename}.jpg`))) {
        artifact.thumbnail_url = `${artifact.media_url}.jpg`;
      }
    }
    ...
    if (existsSync(mediaDir)) {
      ...
      for (const entry of readdirSync(mediaDir)) {
        ...
        if (stat.isFile()) {
          const destination = join(mediaOutputDir, entry);
          if (!existsSync(destination) || statSync(destination).size !== stat.size || Math.abs(statSync(destination).mtimeMs - stat.mtimeMs) > 1) {
            cpSync(src, destination, { preserveTimestamps: true });
          }
        }
      }
    }
    ...
    const summary = {
      run_id: run.run_id,
      title: run.title,
      logline: run.logline || [...currentAnswerByModel.values()].map((answer) => answer.structured_prompt).find((concept) => concept && typeof concept === 'object' && 'logline' in concept && concept.logline)?.logline || '',
      preview: [...artifacts].reverse().find((artifact) => artifact.status === 'generated' && (artifact.thumbnail_url || artifact.media_url)) ?? null,
      status: run.status,
