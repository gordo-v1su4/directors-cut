...
- [x] **B3b** — Phase 2 comparison UI: `/comparisons` route, comparison table with prompt-only vs reference-assisted columns, versioned artifact cells, reference-image strips, keep/remix/reject/resend actions, empty states for missing real answers. Built from `docs/comparison-lab-requirements.md`; `bun run check` clean; verified at http://127.0.0.1:5190/comparisons. Real answers still need to be captured via Raycast Script Commands.
